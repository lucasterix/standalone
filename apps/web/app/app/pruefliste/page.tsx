...
import { api, getOrgId, type JournalZeile } from "@/lib/client";
    ...
    <main className="mx-auto max-w-4xl space-y-5 px-6 py-7">
      ...
      {zeilen && zeilen.length === 0 ? (
        <div className="rounded-3xl border border-brand-200 bg-white px-8 py-16 text-center shadow-sm">
          <span aria-hidden className="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-status-good-bg text-2xl text-status-good">✓</span>
...
function Karte({ j, offen, onToggle, onEntscheiden }: {
  ...
  return (
    <li className="overflow-hidden rounded-2xl border border-sand-200 bg-white shadow-sm">
      <button
        type="button"
        onClick={onToggle}
        aria-expanded={offen}
      ...
      >
        ...
        <span className={
          ...
          (einnahme ? "text-status-good" : "text-sand-900")
        }>
          {einnahme ? "+" : "−"}{j.betrag} €
        </span>
          ...
          <div className="grid gap-5 sm:grid-cols-[200px_1fr]">
            <div>
              ...
              <div className="mt-2 rounded-xl border border-sand-200 bg-white px-4 py-3">
                <p className="tnum font-semibold text-sand-900">
                  {einnahme ? j.soll : j.haben} · Bank
                </p>
                <p className="mt-0.5 text-[12px] text-sand-500">🔒 fixiert</p>
              </div>
            </div>
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-wider text-sand-500">
              ...
              <input
                ...
                className="tnum mt-2 w-40 rounded-xl border border-sand-300 bg-white px-4 py-3 text-[15px] font-semibold focus:border-brand-600 focus:outline-none"
              />
              ...
              <div className="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-sand-200 pt-4">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="rounded-xl border border-sand-300 px-4 py-2 text-[13px] font-semibold text-sand-700 transition hover:border-status-crit hover:text-status-crit"
                  >
                    Ablehnen
                  </button>
                  <button
